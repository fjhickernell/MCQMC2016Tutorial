...
\newcommand{\FJHLessonTwoHalf}{The deterministic discrepancy when $\cf$ is an RKHS  
has 
a simple, explicit form involving three factors. }	

\newcommand{\FJHLessonFourteen}{The formula for the Bayesian discrepancy mimics that 
for the deterministic discrepancy when $\cf$ is an RKHS. }	
...
\begin{description}
	...
	depends only on the sample fraction, $n/N$, but in other cases it will also depend on the 
...
\end{align}
where $\std_{\hnu}(g(\hnu)) := \sqrt{\{\Ex_{\hnu}[g(\hnu)]^2 \}- \{\Ex_{\hnu}[g(\hnu)]\}^2}$ , 
i.e., 
...
linear functional.  If $L(f) \ne 0$ for any $f$, then $\cf$ is assumed to contain  
...
\eqref{FJH:eq:INT} and \eqref{FJH:eq:AppxINT}, for all $f \in \cf$ and $\nu - \hnu  \in 
\cm_\bot$, the cubature 
error 
can be written as a 
single integral:
...
\cx \times \cx \to \R$ satisfying these two properties \cite[Sec.\ 1]{Aro50}:
\begin{equation}
...
\D\bsx$.  Suppose that one starts with $\mu = \int_{\cx} g(\bst) \, 
\D\bst$ and can identify an onto mapping $\bszeta: [0,1]^d \to \cx$ with finite
...
\mu = \lim_{d \to \infty} \mu^{(d)}, \qquad \mu^{(d)} = \int_{\cx_d} f^{(d)}(\bsx) \, \nu^{(d)}(\D 
\bsx),
\end{equation} 
where $\cx^{(d)} = \cx_1 \times \cdots \times \cx_d$, $\nu^{(d)}$ is a measure on $\cx^{(d)} $ 
with 
independent marginals $\nu_k$ on $\cx_k$, and $f_1, f_2, \ldots$ are approximations to an 
...
because evaluating $f^{(d)}(\bsx)$ for a single $\bsx$ typically requires $\Order(d)$ 
operations, 
and so $\mu^{(d)}$ requires $\Order(nd)$ operations.
...
- f^{(d_{k-1} )} \bigr)$ decreases as $k$ increases and when $\mu - \mu^{(d_L)}$ is small \cite{
	...
	$\hmu\bigl(f^{(d_k)} - 
f^{(d_{k-1})} \bigr)$ is $\Order(n_k d_k)$, and as $d_k$ increases, the sample size, $n_k$, 
...
$\Var^{\Dt}(f_\fraku)$ to ensure that $\mu - \sum_{\fraku \notin \{\fraku_1,   \ldots, 
	...
	$\hmu(f_{\fraku})$ 
is $\Order(n_\fraku \lvert \fraku \rvert)$.  If the important sets have small cardinality, 
