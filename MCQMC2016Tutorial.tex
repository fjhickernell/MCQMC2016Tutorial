...
kernel Hilbert spaces has a simple explict form involving three terms. }	
...
\multicolumn{3}{r@{\quad}}{\text{Setting}}   & \Rn & \Dt & \Rn\\
\multicolumn{3}{r@{\quad}}{\text{Possible }\hnu} &  \textup{IID} & 
\textup{Sobol'}&  \textup{Scr.\ Sobol'}\\
 \hnu & \mu - \hmu & \disc & \Order(n^{-1/2}) & 
\Order(n^{-1+\epsilon}) & 
\Order(n^{-3/2+\epsilon})\\
\hline 
\textup{Worst} & \Order(1) & \algn &  \Order(n^{1/2}) & \\
\textup{IID} & \Order(n^{-1/2}) & \algn & \Order(1) & \\
\textup{Worst Sobol'}& \Order(n^{-1+\epsilon}) & \algn & 
\Order(n^{-1/2+\epsilon}) & \Order(1) 
&\Order(n^{1/2+\epsilon})\\
\textup{Scr.\ Sobol'} &\Order(n^{-3/2+\epsilon}) & \algn 
&\Order(n^{-1+\epsilon})&\Order(n^{-1/2+\epsilon})&\Order(1) 
\end{array}
\end{equation*}
\end{table}
