...
\newcommand{\FJHLessonSix}{The trio identity has four versions, depending on whether the 
integrand is deterministic or Bayesian and whether the sampling measure is deterministic or 
random.  }

\newcommand{\FJHLessonSeven}{Although it has traditionally been excluded from cubature 
error analysis, the confounding helps explain why the cubature error may decay to zero 
much faster or more slowly than the discrepancy.  }

\newcommand{\FJHLessonNine}{Bayesian cubature has a purely deterministic alternative 
interpretation.  }


	
\newcommand{\FJHQOne}{How do good sampling measures, $\hnu$, make 
the error, $\mu - \hmu$, smaller?}
...
\newcommand{\FJHLessonTwo}{Quasi-Monte Carlo methods replace IID data sites by 
	...
	a faster rate than the case of IID sampling. }

\newcommand{\FJHLessonTwoHalf}{The deterministic discrepancy for reproducing 
	kernel Hilbert spaces has a simple, explict form involving three factors. }	
...
of integrand values.  Quasi-Monte Carlo methods choose data sites that are not independent 
and identically distributed to make the approximations more accurate.  This article describes 
the trio identity for cubature error, which is a product of three factors: the variation, the 
discrepancy, and the 
confounding.  Each of these factors measures a  
different contribution to the cubature error.  The trio identity has different versions, depending 
on whether  the integrand is deterministic or Bayesian and whether the sampling measure is 
deterministic or random.  Both theory and examples are used to show how low discrepancy 
sampling may reduce cubature error and how rewriting the integral in terms of a different 
integrand may also reduce the cubature error.} 
...
\emph{probability} measure.  Here, $\mu$ represents the weighted average of the integrand or 
$\Ex[f(\bsX)]$, where the random variable $\bsX$ has probability measure $\nu$.  Monte 
...
\end{list}
\noindent We present some known results and highlight fifteen key lessons that are 
interspersed throughout this 
article and repeated at the end.
...
\end{align}
where $\std_{\hnu}(g(\hnu)) := \sqrt{\Ex_{\hnu}[g(\hnu)]^2 - \{\Ex_{\hnu}[g(\hnu)]\}^2}$ , i.e., 
the standard deviation with 
respect to the random sampling 
measure.  Direct calculation yields
\begin{equation*}
...
\hnu)$.  For a web survey the absolute cubature error in \eqref{FJH:eq:surveydettrio} is 
$\corr\bigl(f, 
...
\disc^{\Rn\Ba}(\nu - \hnu) = \sqrt{\Ex_{\hnu}  \bigl(c_0 - 
		2 \bsc^T \bsw + \bsw ^T \mC \bsw \bigr)},\\
	\algn^{\Rn\Ba}(f,\nu - \hnu) :=\frac{ \int_{\cx} 
		f(\bsx) \, (\nu - \hnu)(\D\bsx)}{s  \sqrt{\Ex_{\hnu}  \bigl(c_0 - 
			2 \bsc^T \bsw + \bsw ^T \mC \bsw \bigr)}}.
	\end{gather}
\end{subequations}
...
the integrand be 
	...
	randomly from $\cm_{\textup{S}}$ according to some probability distribution.  For the  
...
$\{\bsz_i\}_{i=1}^n$ are deterministic data sites, and $\bspsi :\cx \to \cx$ is a random 
function.  For example, for IID sampling on $\cx$, $\bspsi(\bsx) = \bsDelta_\bsx$, where 
...
where $\bsDelta \sim \cu[0,1]^d$.  For randomly scrambled digital sequences there also 
...
\bst) \in \cx \times \cx$ suppose that  $\bigl(\bspsi(\bsx), \bspsi(\bst) \bigr)$ and 
$\bigl(\bspsi(\bspsi(\bsx)), \bspsi(\bspsi(\bst)) \bigr)$ have the same probability measure.

Define a filtered version of the covariance kernel as $\filC(\bsx,\bst) = \Ex_{\bspsi} 
...
\begin{subequations} \label{FJH:eq:filCvecmatdef}
	\begin{gather}
	 \Ex_{\bspsi}(c_0)  = c_0 =  \int_{\calX^2} C(\bsx,\bst) \, \nu(\D \bsx) \nu(\D 
	 \bst) = 
	 \int_{\calX^2} \filC(\bsx,\bst) \, \nu(\D \bsx) \nu(\D \bst) =: \filc_0, \\
	 \Ex_{\bspsi}(\bsc) = 
	\biggl( \int_{\cx} 
	\Ex_{\bspsi}\bigl[C(\bspsi(\bsz_i),\bst)] \,\nu(\D \bst) \biggr)_{i=1}^n = 
	\biggl( \int_{\cx} 
	\filC(\bsz_i,\bst) \,\nu(\D \bst) \biggr)_{i=1}^n = : \filbsc, \\
	 \Ex_{\bspsi}(\mC)  = \biggl(  \Ex_{\bspsi} 
...
\begin{equation*}
\bigl[\disc^{\Rn}(\nu - \hnu)\bigr]^2 \le \Ex_{\hnu} \bigl[\disc^\Dt(\nu - \hnu)\bigr]^2 = 
\Ex_{\hnu} \bigl[\disc^\Ba(\nu - 
...
clear error 
decay rate of $\Order(n^{-1 + \epsilon})$ for low discrepancy sampling for the option 
pricing problem with dimension $12$.  However, Fig.\ \ref{FJH:fig:unwtdiscdiffpts} shows 
that the discrepancy for these scrambled Sobol' points does not decay like $\Order(n^{-1 
...
\section{Variable Transformations to Reduce Cubature Error}
...
Importance sampling is a method for improving the accuracy of IID Monte Carlo 
methods, but it can also be useful for quasi-Monte Carlo methods, which are based on 
low discrepancy sequences.  We describe importance sampling disguised as a variable 
transformation.  Since such sequences are typically generated to mimic the 
uniform measure on 
the unit cube, $[0,1]^d$, we consider the integral $\mu = \int_{[0,1]^{d}} g(\bst) \, 
\D\bst$.  If $\zeta: [0,1] \to [0,1]$ is a differentiable, increasing, onto function, then $\mu 
= \int_{[0,1]^{d}} f(\bsx) \, 
\D\bsx$, where $f(\bsx) = g\bigl(\zeta(x_1), \ldots, \zeta(x_d) \bigr) \zeta'(x_1) \cdots 
\zeta'(x_d)$.  
If $\zeta$ can be chosen to make $\Var^\Dt(f) < \Var^\Dt(g)$, then the cubature error 
for approximating the integral of $f$ will generally be smaller than for approximating the 
integral of $g$.  The choice of variable 
transformation is more of an art than a science.
...
control variate coefficientm $\bsbeta$, is subtle for low discrepancy sampling measures 
...
the cubature error, it does not in itself answer the question of how many samples are 
...
error bounds.  These can be used to determine how large $n$ must be to make the error 
small enough.

For IID Monte Carlo the Central Limit Theorem may be used to construct an approximate 
confidence interval for $\mu$, however, this approach relies on believing that the 
asymptotic limit has been reached and a reliable upper bound on the standard deviation 
of 
the integrand has been computed from the integrand values.  There have been recent 
...
on the kurtosis or estimating the kurtosis from data.  The Central Limit Theorem may be 
replaced by the  Berry-Esseen inequality, which is valid for finite samples.  
...
\FJHLessonZero \FJHLessonSix  \FJHLessonTwoHalf \FJHLessonSeven \FJHLessonNine
