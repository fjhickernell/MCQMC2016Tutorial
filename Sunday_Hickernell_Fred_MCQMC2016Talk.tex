...
\begin{frame}
	\frametitle{August 12, 2016 News}
	\vspace{-3ex}
	\includegraphics[width = 10cm]{ProgramsImages/WSJ_MonteCarlo.pdf}
	
	\bigskip
	
	\hspace{1.7cm}\includegraphics[width = 10cm]{ProgramsImages/WSJ_ArtOwen.pdf}
	
	\hfill \hfill \alert{Can a more clever brute-forcing give us a better picture?}
...
\begin{frame}
   ...
   \Var(f) & =  \text{\alert{variation} of the integrand} \ge 0 \\
	\disc(\nu - \hnu) & =  \text{\alert{discrepancy} of the sampling measure }\\[-0.5ex]
	 &  \qquad \qquad  \text{from the probability measure} \ge 0\\[-0.5ex]
	 &  \qquad \qquad \qquad \qquad\alert{\text{expect this to track error as } n \to \infty} \\ 
	\algn(f,\nu - \hnu) & =  \text{\alert{alignment} between the integrand and} \\[-0.5ex]
	&  \qquad \qquad  \text{the difference between the two measures} \\[-0.5ex]
 &\qquad \qquad \qquad \qquad \text{\alert{should not be ignored}}
	\end{align*}
...
\hspace{2ex}\parbox{4.2cm}{\raggedright \alert{Low discrepancy sampling reduces cubature error}}}
	&
...
%	\vspace{-1ex}	
	...
	\centerline{\redroundmathbox{\mu - \hmu  = \begin{cases} 
			...
			\std(f) & \text{web} \\[2ex]
			\frac{- \corr\bigl(f, \bbone_{\{\vx_i\}_{i=1}^n}\bigr) }{\std_{\hnu} \bigl( \corr\bigl(f, \bbone_{\{\vx_i\}_{i=1}^n}\bigr)\bigr)} \, \sqrt{\frac{1-n/N}{\alert{n}(1-1/N)}} \, \std(f)  & \text{simple random}
	\end{cases}}}
	\vspace{-3ex}
	\begin{itemize}
		
		\item The \alert{vast majority} of designs, $\{\vx_i\}_{i=1}^n$, have $\corr\bigl(f, \bbone_{\{\vx_i\}_{i=1}^n}\bigr) = \Order(1/\sqrt{N})$, but your web survey is likely one of the \alert{awful minority} that does not
...
\begin{frame} \frametitle{Multivariate Normal Probability}
	...
	\begin{tabular}{m{4.5cm}m{7cm}}
		...
		\alert{Randomized sampling, $\hnu$, may avoid the awful minority }}
		&
...
\begin{frame}
	...
	\begin{tabular}{m{4.5cm}m{7cm}}
		...
		\bigskip
	Scrambled Sobol' does not achieve $\Order(n^{-3/2+\epsilon})$ convergence because $f$ is \alert{not smooth enough}.
		
		\bigskip
		
		$f$ is not even smooth enough for $\Order(n^{-1+\epsilon})$ convergence, except by a delicate argument \smallcites{GriKuoSlo10, GriKuoSlo16}
...
\begin{frame}<1>[label=BayesCub]
	...
	\only<1>{\qquad \alert{Nice:\quad}$
		...
		\vy, \, C_{\vtheta} & \vw & \hmu & \text{width}& \hvtheta  \tabularnewline
		\toprule
		47 \vy, \, 29 C_{\vtheta} & \vw & 47 \hmu & 47 \text{width} & \hvtheta 
		\end{array}$}
	\only<3->{%
		\\[-1ex]
