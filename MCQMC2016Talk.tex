...
\begin{frame}
	...
	\centerline{\redroundmathbox{\mu - \hmu  = 
			\underbrace{-\corr\bigl(f, \bbone_{\{\vx_i\}_{i=1}^n}\bigr)}_{\textstyle\algn(f,\nu - \hnu)}
			\underbrace{ \sqrt{\frac{1-n/N}{\alert{n/N}}}}_{\textstyle\disc(\nu - \hnu)}
			\underbrace{ \std(f)}_{\textstyle\Var(f)} }}
		\vspace{-2ex}
		\begin{itemize}
...
\begin{frame}
	...
	\end{align*}
	\centerline{\redroundmathbox{\mu - \hmu = \cos(f,\text{error rep}) \disc(\nu - \hnu) \norm[\cf]{f-L(f)}}
	}
\end{frame}
...
\begin{frame}
	...
	\only<1>{\begin{tabular}{>{\flushright}m{5cm}>{\centering}m{6cm}}
		...
		\end{tabular}}
\only<2>{	\centerline{\redroundmathbox{\mu - \hmu = \cos(f,\text{error rep}) \disc(\nu - \hnu) \norm[\cf]{f-f(\vone)}}}
	\vspace{-4ex}
	\begin{itemize}
	...
	\frametitle{Randomized Trio Identity}
		...
		\uncover<2->{\redroundmathbox{\mu - \hmu} &= \int_{\cx} f(\vx) \, (\nu - \hnu)(\dif\vx) \\
			& =  \redroundmathbox{\displaystyle%
				...
				\underbrace{\sup_{\substack{f \in \cf \\ f \ne 0= L(f)}}  \frac{\RMSE(f, \nu - \hnu)}{\norm[\cf]{f}}}_{\textstyle\disc(\nu - \hnu)} \, \underbrace{\norm[\cf]{f-L(f)}}_{\textstyle\Var(f)}} }
		\end{align*}
		\vspace{-3ex}
		\uncover<2->{
			\begin{itemize}
				\item $\Ex\abs{\algn(f,\nu - \hnu)}^2  \le 1$ by the definitions of $\disc(\nu - \hnu), \Var(f)$
				
				\item $\disc^{\text R}(\nu - \hnu)  \le \sqrt{\Ex\abs{\disc^{\text D}(\nu - \hnu)}^2}$
			\end{itemize}
		}
...
\redroundmathbox{\mu - \hmu} 
		& =  \redroundmathbox{\displaystyle%
			\underbrace{\frac{ \sqrt{n}\int_{\cx} f(\vx) \, (\nu - \hnu)(\dif\vx)}{\norm[L^2]{f - \mu}}}_{\textstyle \algn(f,\nu - \hnu)} \, 
			\underbrace{\frac{1}{\sqrt{n}}}_{\textstyle\disc(\nu - \hnu)} \, \underbrace{\norm[L^2]{f - \mu}}_{\textstyle\Var(f)}}  \\
		& \qquad \Ex[\algn(f,\nu - \hnu)]  = 0, \qquad \hmu \text{ is unbiased} \\
	& \qquad \var\bigl(\algn(f,\nu - \hnu) \bigr)  = 1
	\end{align*}
...
\begin{frame} \frametitle{Multivariate Normal Probability}
	...
	\centerline{\redroundmathbox{\mu - \hmu = 
    \begin{Bmatrix} \cos(f,\text{error rep}) & \disc^{\text{D}}(\nu - \hnu) \\
				\algn^{\text{R}}(f,\nu - \hnu) &  \disc^{\text{R}}(\nu - \hnu) \end{Bmatrix} \Var(f) }  }
