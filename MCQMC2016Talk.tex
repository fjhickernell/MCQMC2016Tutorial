...
\\[2ex]
Joint work with Llu\'is Antoni Jim\'enez Rugama and the rest of the GAIL team \\[2ex]
...
\begin{frame}
	...
	\frametitle{\textbf<1-2>{Error Analysis} for \textbf<3>{Quasi-Monte Carlo Methods}}
	\vspace{-7ex}
{\large	\begin{gather*} \mu := \Ex[f(\vX)] := \int_{\cx} f(\vx) \, \nu(\dif\vx) \approx \hmu := \sum_{i=1}^n f(\vx_i) w_i \\
	\redroundmathbox{\mu - \hmu =\, ?} 
	 \end{gather*}
	 \vspace{-8ex}
	\begin{center}
		\begin{tabular}{>{\centering}p{0.45\textwidth}>{\centering}p{0.5\textwidth}}
\uncover<1->{\alert{Reality} & \alert{\textbf<1-2>{Error Analysis} (Theory)} \tabularnewline 
What is &  What we think it should be \\[1ex]}
\uncover<2->{Because we are rarely certain that our assumptions hold}
		\end{tabular}
	\end{center}
\uncover<3->{\alert{Quasi-Monte Carlo Methods} \\ [-1ex]
	\begin{itemize}
...
\only<2>{\\ & \hspace{3cm} \qquad \text{\alert{often ignored, but shouldn't be}}}}
	\end{align*}
	
	\only<1-3>{\uncover<3>{\vspace{-2ex}\begin{itemize}
			\item \alert{Reduce} error by clever sampling, $\hnu$
			\item \alert{Reduce} error by rewriting the problem while leaving $\mu$ unchanged
			\item Improve \alert{understanding} of error by modifying our definitions of $\disc$ and $\Var$
			\item \alert{Determine} the sample size $n$ needed to make $\abs{\mu - \hmu} \le \varepsilon$
		\end{itemize}}}
...
\begin{frame}
	...
	(\cm, \norm[\cm]{\cdot}) & = \text{normed vector space of signed measures, includes } \delta_{\vt} \ \forall \vt \in \cx\\
...
\begin{frame}
	...
	\end{tabular}
	\vspace{-4ex}
\[ \mu = \mu_f = \frac 1N \sum_{j=1}^N f(j) , \qquad \cov(f,g) = \frac 1N \sum_{i=1}^N [f(j) - \mu_f][g(j) - \mu_g]\]
	\centerline{\redroundmathbox{\mu - \hmu  = 
		...
		\begin{itemize}
			...
			\item Discrepancy may be large if the sample fraction is small \smallcite{Men16a}
...
\begin{frame}
	...
	\end{align*}
	\only<1>{\vspace*{-1.8ex}
		\centerline{\includegraphics[width = 6.7cm]{ProgramsImages/L2Kernel.eps}}}
...
\begin{frame}<1>[label=BayesCub]
	...
	\begin{array}{rcl}
		...
		\text{with } 99\% \text{ confidence}&& \uncover<3->{\text{if }  \Var^{\Dt}(f - \tf_{\vy}) \le \frac{2.58}{\alert{n}} \Var^{\Dt}(\tf_{\vy}) }\tabularnewline
		 ...
		 \uncover<2->{\text{replace } C_{\vtheta} \text{ by } K_{\vtheta}}
		  \tabularnewline 
		  = 2.58 \disc^{\Ba}(\nu - \hnu) \Var^{\Ba}(f) &&  \uncover<3->{=\frac{2.58}{\alert{n}}\disc^{\Dt}(\nu - \hnu) \Var^{\Dt}(\alert{\tf_{\vy}}) } \tabularnewline
		  ...
		  \mC_{\hvtheta}^{-1} \vc_{\hvtheta} & \leftarrow \vw \uncover<2->{\rightarrow}& \uncover<2->{\text{replace } C_{\vtheta} \text{ by } K_{\vtheta}} \tabularnewline
	  \argmin_{\vtheta} \frac{\vy^T \mC_{\vtheta}^{-1} \vy}{[\det(\mC_{\vtheta}^{-1})]^{1/n}} & \leftarrow 	\hvtheta  \uncover<2->{\rightarrow} &  \uncover<2->{\text{replace } C_{\vtheta} \text{ by } K_{\vtheta}} \tabularnewline
	  \text{estimate $s$ and $\vtheta$ by MLE} & & \uncover<3->{ = \argmin_{\vtheta}  \vol \bigl(\{ \vz \in \reals^n : \tabularnewline 
	  	&& \qquad\quad \Var^{\Dt}_{\vtheta}(\tf_{\vz}) \le \Var^{\Dt}_{\vtheta}(\tf_{\vy})\} \bigr ) }
	\end{array}
	\]
	\only<1-3>{\qquad \alert{Nice:\quad}$
		\begin{array}{cccccc}
		\vy, \, \mC_{\vtheta} & \vw & \hmu & \text{width}& \hvtheta & \hs \tabularnewline
		\toprule
		47 \vy, \, 29 \mC_{\vtheta} & \vw & 47 \hmu & 47 \text{width} & \hvtheta &   47\hs/\sqrt{29}
		\end{array}$}
	\only<4->{%
		\\[-1ex]
...
\begin{frame} \frametitle{Multivariate Normal Probability}
	...
	\begin{tabular}{>{\raggedleft}m{4.5cm}>{\centering}m{7cm}}
		...
		\only<1>{Use a product Mat\'ern kernel with modest smoothness:
			\begin{multline*}C(\vx,\vt) \\
			= \prod_{k=1}^d \me^{-\alpha\abs{x_k - t_k}}(1 + \alpha\abs{x_k - t_k})\end{multline*}}
		\only<2-3>{Improved convergence for smoother integrands using Bayesian cubature with scrambled Sobol' data sites\\
		\vspace{3ex}	
...
\begin{frame}
	...
	\vspace{-4ex}
	\ocite{SloWoz98} showed how inserting decaying coordinate weights rescues the $\Order(n^{-1+\epsilon})$ convergence rate \alert{independent of dimension}.  \ocite{CafMorOwe97} explained that quasi-Monte Carlo methods work well for \alert{low effective dimension} even when $d$ is large.
...
Niederreiter low dimensional quote

finance example convergence versus MVN convergence
O'Hagan quote
...
\begin{frame}
	...
	f  \sim \GP (0, s^2 C_{\vtheta}) & & \uncover<2->{f \in \cf \text{ w/ reproducing kernel } K_{\vtheta}} \tabularnewline \toprule
	\multicolumn{3}{c}{\abs{\mu - \hmu} \le \text{ width}} \tabularnewline
	\text{with probability } 99\% && \uncover<2->{\text{if }  \Var^{\Dt}(f - \tf_{\vy}) \le \fC\Var^{\Dt}(\tf_{\vy}) }\tabularnewline
	2.58\sqrt{\bigl (c_{\hvtheta,0} - \vc_{\hvtheta}^T  \mC_{\hvtheta}^{-1} \vc_{\hvtheta}\bigr) \, \frac {\vy^T \mC_{\hvtheta}^{-1}  \vy}{\alert{n}} } &
	\leftarrow\text{width}\uncover<2->{\rightarrow}&
	\uncover<2->{\alert{\fC} \sqrt{\bigl(k_{\hvtheta,0} - \vk_{\hvtheta}^T  \mK_{\hvtheta}^{-1} \vk_{\hvtheta}\bigr) \bigl(\vy^T \mK_{\vtheta}^{-1}  \vy \bigr)} }
	\tabularnewline 
	= 2.58 \disc^{\Ba}(\nu - \hnu) \Var^{\Ba}(f) &&  \uncover<2->{=\alert{\fC}\disc^{\Dt}(\nu - \hnu) \Var^{\Dt}(\alert{\tf_{\vy}}) } \tabularnewline
	\multicolumn{3}{c}{\vy = \bigl(f(\vx_i)\bigr)_{i=1}^n} \tabularnewline
	&& \uncover<2->{\tf_{\vy} = \text{minimum norm interpolant}} \tabularnewline
	\mC_{\hvtheta}^{-1} \vc_{\hvtheta} & \longleftarrow \vw \uncover<2->{\longrightarrow}& \uncover<2->{\mK_{\hvtheta}^{-1} \vk_{\hvtheta}} \tabularnewline
	\argmin_{\vtheta} \frac{\vy^T \mC_{\vtheta}^{-1} \vy}{[\det(\mC_{\vtheta}^{-1})]^{1/n}} & \longleftarrow 	\hvtheta  \uncover<2->{\longrightarrow} &  \uncover<2->{\argmin_{\vtheta} \frac{\vy^T \mK_{\vtheta}^{-1} \vy}{[\det(\mK_{\vtheta}^{-1})]^{1/n}}} \tabularnewline
	\text{estimate $s$ and $\vtheta$ by MLE} & & \uncover<2->{ = \argmin_{\vtheta}  \vol \bigl(\{ \vz \in \reals^n : \tabularnewline 
		&& \qquad\quad \Var^{\Dt}_{\vtheta}(\tf_{\vz}) \le \Var^{\Dt}_{\vtheta}(\tf_{\vy})\} \bigr ) }
	%\hs = \sqrt{\frac {\vy^T \mC_{\vtheta}^{-1}  \vy}n }
	\end{array}
	\]
	\vspace*{-4ex}
	\uncover<3>{\begin{itemize}
			%\item $C_{\vtheta} = K_{\vtheta} \implies \hmu^{\Dt} = \hmu^{\Ba}, \text{ but }\RMS_{f} \Var^{\Dt}_{\vtheta}(f) \ge \sqrt{n} \Var^{\Ba}_{\vtheta}(f)$!!!!
			\item generally requires $\Order(n^3)$ operations to compute $\mC_{\vtheta}^{-1}$ 
			\item susceptible to ill-conditioning for smoother kernels (faster convergence)
